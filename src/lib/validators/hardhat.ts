...
import { EOL } from "os";
import type { Deployment, GasReporterOutput, MethodDataItem } from "../../types.js";
...
export class HardhatGasReporterOutputValidator {
  ...
  public validateOutputObject(
    output: GasReporterOutput,
  ...
  public validateDeploymentDataItem(
  ...
  private _checkResult(result: ValidatorResult, sourceFilePath: string) {
    if (result.errors.length) {
      ...
      throw new Error(
        `[hardhat-gas-reporter] Unexpected JSON report format in ${sourceFilePath}. ` +
          `Reported JSON validation error was: ${EOL}${errors}`
      );
    }
  }
}
